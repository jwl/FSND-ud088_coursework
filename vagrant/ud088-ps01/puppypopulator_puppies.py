...
# def setup_module():
    ...
    # engine = create_engine('sqlite:///test_puppies.db')
...
def populate_database():
    ...
    #Add Shelters
    shelter1 = Shelter(name = "Oakland Animal Services", address = "1101 29th Ave", city = "Oakland", state = "California", zipCode = "94601", website = "oaklandanimalservices.org", maximum_capacity = 1024, current_occupancy = 0)
    session.add(shelter1)

    shelter2 = Shelter(name = "San Francisco SPCA Mission Adoption Center", address="250 Florida St", city="San Francisco", state="California", zipCode = "94103", website = "sfspca.org", maximum_capacity = 1024, current_occupancy = 0)
    session.add(shelter2)

    shelter3 = Shelter(name = "Wonder Dog Rescue", address= "2926 16th Street", city = "San Francisco", state = "California" , zipCode = "94103", website = "http://wonderdogrescue.org", maximum_capacity = 1024, current_occupancy = 0)
    session.add(shelter3)

    shelter4 = Shelter(name = "Humane Society of Alameda", address = "PO Box 1571" ,city = "Alameda" ,state = "California", zipCode = "94501", website = "hsalameda.org", maximum_capacity = 1024, current_occupancy = 0)
    session.add(shelter4)

    shelter5 = Shelter(name = "Palo Alto Humane Society" ,address = "1149 Chestnut St." ,city = "Menlo Park", state = "California" ,zipCode = "94025", website = "paloaltohumane.org", maximum_capacity = 1024, current_occupancy = 0)
    session.add(shelter5)

    #Add adopters
    adopter1 = Adopter(name = "Nami")
    session.add(adopter1)

    adopter2 = Adopter(name = "Quinn")
    session.add(adopter2)

    adopter3 = Adopter(name = "Kog'Maw")
    session.add(adopter3)

    adopter4 = Adopter(name = "Vi")
    session.add(adopter4)

    adopter5 = Adopter(name = "Nautilus")
    session.add(adopter5)

    adopter6 = Adopter(name = "Poppy")
    ...
    for i,x in enumerate(male_names):
        ...
        shelter = session.query(Shelter).\
                filter(Shelter.shelter_id == new_puppy.shelter_id).one()
